...
namespace PharmacyManager.UserControlMain
{
    public partial class ThemThuocMoi : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void Setgt(string MaThuoc,string TenThuoc,string HoatChat,int GiaNhap,int GiaSauThue,int GiaHop,int Hop,int Vi,int GiaVi,int Vien,int GiaVien,int Goi,int GiaGoi,int QuyDinh,float THSuDung)
        {
            txt_MaThuoc.Text = MaThuoc;
            txt_MaThuoc.Enabled = false;
            txt_TenThuoc.Text = TenThuoc;
            txt_HoatChat.Text = HoatChat;
            txt_GiaNhap.Text = GiaNhap.ToString();
            txt_GiaSauThue.Text = GiaSauThue.ToString();
            txt_GiaHop.Text = GiaHop.ToString();
            txt_SoLuongMoiHop.Text = Hop.ToString();
            if(Vien==1)
            {
                cbb_LoaiThuoc.SelectedIndex = 0;
                txt_GiaMoiVien.Text = GiaVien.ToString();
                cbb_SoLuongVienMoiVi.SelectedIndex = Vi - 1;
                txt_GiaMoiVi.Text = GiaVi.ToString();
            }
            else if(Goi==1)
            {
                cbb_LoaiThuoc.SelectedIndex = 1;
                txt_GiaMoiVien.Text = GiaGoi.ToString();
            }
            cbb_QuyDinh.SelectedIndex = QuyDinh;
            int nam_1 = (int)THSuDung;
            string t = THSuDung.ToString();
            float setthang;
            if ((float)nam_1 == THSuDung)
                setthang = 0;
            else
            {
                t = t.Remove(0, 2);
            ...
            cbb_NamSuDung.SelectedIndex = nam_1 - 1;
            cbb_thangSuDung.SelectedIndex = thang_1;
            simpleButton2.Text = "CẬP NHẬT";
        }
        ...
        private void btn_NhapThuoc_Click(object sender, EventArgs e)
        {
            ...
            int Goi=0;
            int GiaGoi=0;
            int QuyDinh = cbb_QuyDinh.SelectedIndex;
            if (cbb_LoaiThuoc.SelectedIndex == 0)
            {
                Vien=1;
            ...
            if (txt_MaThuoc.Enabled == false)
            {
                float THSuDung = float.Parse(ThoiHanSuDung);
                t.CapNhatThuoc(MaThuoc, TenThuoc, HoatChat, GiaNhap, GiaSauThue, GiaBan, THSuDung, Hop, Vi, GiaVi, Goi, GiaGoi, Vien, GiaVien, QuyDinh);
            ...
            else
            {
                try
                {
                    bool check = t.NhapThuocMoi(MaThuoc, TenThuoc, HoatChat, GiaNhap, GiaSauThue, GiaBan, ThoiHanSuDung, Hop, Vi, GiaVi, Goi, GiaGoi, Vien, GiaVien, QuyDinh);
                    if (check)
                    {
