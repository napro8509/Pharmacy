...
namespace DAL
{
    public class DAL_THUOC : BDConnect
    {
        ...
        public DataSet Get_THUOC(int LoaiThuoc)               
        {
            try
            {
                SqlDataAdapter da;
                ...
                if (LoaiThuoc == 0)
                    sqlstr += " where QuyDinh=0";
                else if(LoaiThuoc==1)
                    sqlstr += " where QuyDinh=1";
                else if (LoaiThuoc == 2)
                    sqlstr += " where QuyDinh=2";
                else if (LoaiThuoc == 3)
                    sqlstr += " where QuyDinh=3";
        ...
        public bool NhapThuocMoi(string MaThuoc,string TenThuoc,string HoatChat,int GiaNhap,int GiaSauThue,int GiaBan,string ThoiHanSuDung,int Hop,int Vi ,int GiaVi ,int Goi ,int GiaGoi ,int Vien ,int GiaVien,int QuyDinh)
        {
            _cn.Open();

            string SQL = "INSERT INTO THUOC (MaThuoc, TenThuoc , HoatChat ,GiaNhap ,GiaSauThue ,GiaBan ,ThoiHanSuDung ,Hop ,Vi ,GiaVi ,Goi ,GiaGoi ,Vien ,GiaVien,QuyDinh) VALUES ";
            SQL += "('" + MaThuoc + "','" + TenThuoc + "','" + HoatChat + "'," + GiaNhap + "," + GiaSauThue + "," + GiaBan + "," + ThoiHanSuDung + "," + Hop + "," + Vi + "," + GiaVi + "," + Goi + "," + GiaGoi + "," + Vien + "," + GiaVien+"," +QuyDinh+ ")";
        ...
        public bool CapNhatThuoc(string MaThuoc, string TenThuoc, string HoatChat, int GiaNhap, int GiaSauThue, int GiaBan, float ThoiHanSuDung, int Hop, int Vi, int GiaVi, int Goi, int GiaGoi, int Vien, int GiaVien, int QuyDinh)
        {
            _cn.Open();

            string SQL = "UPDATE THUOC ";
            SQL += "SET TenThuoc='" + TenThuoc + "', HoatChat='" + HoatChat + "', GiaNhap="+GiaNhap+", GiaSauThue="+GiaSauThue;
            SQL += ", GiaBan=" + GiaBan + ", ThoiHanSuDung=" + ThoiHanSuDung + ", Hop=" + Hop + ", Vi=" + Vi + ", GiaVi=" + GiaVi + ", Goi=" + Goi + ", GiaGoi=" + GiaGoi + ", Vien=" + Vien + ", GiaVien=" + GiaVien + ", QuyDinh=" + QuyDinh + "";
